...
DEFINE_RUNTIME_bool(
    cdc_read_wal_segment_by_segment, true,
    "When this flag is set to true, GetChanges will read the WAL segment by segment.");
...
Status GetConsistentWALRecords(
  ...
  do {
    ...
    uint64_t consistent_stream_safe_time_footer = HybridTime::kInvalid.ToUint64();

    if (FLAGS_cdc_read_wal_segment_by_segment) {
      // Read all the ops from the segment to which last_seen_op_id belongs. If last_seen_op_id is
      // the last opid in the segment then ReadReplicatedMessagesInSegmentForCDC will read the next
      // segment.
      read_ops = VERIFY_RESULT(consensus->ReadReplicatedMessagesInSegmentForCDC(
          *last_seen_op_id, deadline, false, *last_readable_opid_index,
          &consistent_stream_safe_time_footer));

      if (consistent_stream_safe_time_footer == HybridTime::kInvalid.ToUint64()) {
        ...
        consistent_stream_safe_time_footer = *consistent_safe_time;
      }
    } else {
      read_entire_wal = true;
      ...
      read_ops = VERIFY_RESULT(consensus->ReadReplicatedMessagesForConsistentCDC(
    ...
    if (!consistent_wal_records->empty()) {
      ...
      if (FLAGS_cdc_read_wal_segment_by_segment &&
          GetTransactionCommitTime(record) <= consistent_stream_safe_time_footer) {
        ...
        *consistent_safe_time = consistent_stream_safe_time_footer;
        break;
      }
    }
