...
std::pair<int64_t, int64_t> PeerMessageQueue::GetCommittedAndMajorityReplicatedIndex() {
  ...
  return std::make_pair(committed_op_id_index, last_replicated_op_id_index);
}
...
Result<ReadOpsResult> PeerMessageQueue::ReadFromLogCacheForCDC(
  ...
  int64_t after_op_index =
      last_op_id_index == 0 ? max<int64_t>(log_cache_.earliest_op_index(), 0)
                         : last_op_id_index;
...
Result<ReadOpsResult> PeerMessageQueue::ReadReplicatedMessagesForConsistentCDC(
  ...
  do {
    ...
    auto pair = GetCommittedAndMajorityReplicatedIndex();
    committed_op_id_index = pair.first;
    last_replicated_op_id_index = pair.second;
...
Result<ReadOpsResult> PeerMessageQueue::ReadReplicatedMessagesInSegmentForCDC(
    OpId from_op_id, CoarseTimePoint deadline, bool fetch_single_entry,
    int64_t* last_commited_index, uint64_t* consistent_stream_safe_time_footer) {
  auto read_ops = ReadOpsResult();

  auto pair = GetCommittedAndMajorityReplicatedIndex();
  auto committed_op_id_index = pair.first;
  auto last_replicated_op_id_index = pair.second;

  // Determine if there are pending operations in RAFT but not yet LogCache.
  auto pending_messages = committed_op_id_index != last_replicated_op_id_index;

  if (last_commited_index) {
    *last_commited_index = committed_op_id_index;
  }
  ...
  auto start_op_id_index =
      from_op_id.index == 0 ? max<int64_t>(log_cache_.earliest_op_index(), 0) : from_op_id.index;
  ...
  auto current_segment_num = VERIFY_RESULT(log_cache_.LookupOpWalSegmentNumber(start_op_id_index));
  if (current_segment_num == static_cast<int64_t>(log_cache_.GetActiveSegmentNumber())) {
    // Read entire WAL.
  ...
  auto consistent_stream_safe_time =
      VERIFY_RESULT(log_cache_.GetConsistentStreamSafeTimeFromSegmentFooter(current_segment_num));
